...
def _select_candidates_for_listing(
    ...
    Semantics:
    - When ``req.ids`` is ``None`` or an empty list, the worker auto-selects
      candidates purely by status/flags, up to ``req.max_items`` rows.
    - When ``req.ids`` is a non-empty list, the selection is restricted to
      that subset of ``parts_detail.id`` values **and** still applies the
      normal status/flag filters (Checked, item_id IS NULL, not frozen/
      cancelled). IDs that do not satisfy these filters are silently ignored.
    - Results are always ordered by ``id ASC`` and limited by
      ``req.max_items``.
    """
