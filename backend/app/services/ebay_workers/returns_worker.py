...
RETURNS_OVERLAP_MINUTES_DEFAULT = 30
RETURNS_INITIAL_BACKFILL_DAYS_DEFAULT = 90
...
async def run_returns_worker_for_account(ebay_account_id: str) -> Optional[str]:
    ...
    try:
        ...
        if not state.enabled:
            logger.info(f"Returns worker: sync disabled for account={ebay_account_id}")
            return None
        ...
        run_id = run.id
        sync_run_id = f"worker_returns_{run_id}"

        overlap_minutes = RETURNS_OVERLAP_MINUTES_DEFAULT
        initial_backfill_days = RETURNS_INITIAL_BACKFILL_DAYS_DEFAULT
        ...
        try:
            ...
            logger.error(f"Returns worker for account={ebay_account_id} failed: {msg}")
            return run_id
