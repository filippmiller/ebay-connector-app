...
async def run_inquiries_worker_for_account(
    ...
    try:
        ...
        logger.info(
            f"HELLO FROM WORKER! [inquiries_worker] Calling fetch_active_ebay_token: account_id={ebay_account_id} "
        ...
        if not decrypted_access_token:
            logger.warning(
                f"Inquiries worker: failed to fetch decrypted token for account {ebay_account_id}"
            )
            return None
