...
(function () {
  ...
  const CSS = `
    ...
    .ai-chat-voice-btn:hover { background: #e0e0e0; }
    ...
    <div id="ai-chat-widget" class="ai-chat-widget ${THEME} ${POSITION}">
      ...
      <div id="ai-chat-window" class="ai-chat-window" style="display: none;">
        ...
        <div class="ai-chat-input-container">
          ...
          <button id="ai-chat-voice-btn" class="ai-chat-voice-btn" aria-label="Voice input" title="Голосовой ввод">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  function initWidget() {
    ...
    const isVoiceSupported = !!(navigator.mediaDevices && navigator.mediaDevices.getUserMedia && window.MediaRecorder);
      ...
      try {
        ...
        mediaRecorder.onstop = async () => {
          ...
          if (audioChunks.length === 0) {
            addMessage('assistant', 'Не удалось записать аудио. Попробуйте еще раз.');
            return;
          }
        ...
        voiceBtn.title = 'Нажмите чтобы остановить запись';
    ...
    async function transcribeAudio(audioBlob) {
      ...
      try {
        ...
        if (transcribedText.trim()) {
          ...
          addMessage('assistant', 'Не удалось распознать речь. Попробуйте говорить четче.');
        }

      } catch (error) {
        hideTyping();
        console.error('Transcription error:', error);
        addMessage('assistant', 'Не удалось распознать речь. Попробуйте еще раз.');
      }
    }
    ...
    console.log("AI Chat Widget loaded. API:", API_URL, "Voice:", isVoiceSupported);
  }
